#class node needed for class queue
class Node:
    def __init__(self,item,next):
        self.item = item
        self.next = next
#class that represents linked queue
class Queue:
    def __init__(self):
        self.rear = None
        self.front = None
...
def radix_sort_str(list):
     max = get_max(list)        #find max (longest) in list, assigns its' length to max
...
def find_rotations(string_list, p):
    ...
    # help function to rotate strings from list p times in O(N * (2l + z))) ~ O(N * (2l) (since z < l) ~ O(N * l)
    ...
    rotated_strings = rotate_string(string_list, p)             #rotates strings in list p times in O(N * l), where N number of items in list, l length of longest string, z is number of turns % l
    matches = check_for_matches(string_list, rotated_strings)   #compares strings in turned strings list to original list in O(N)
    ...
    matches = check_for_matches(string_list,rotate_matches_back)    #check for matches between original list and rotated back match list

    return matches


#compares 2 lists in O(N + M + N * n + M * m + (N * n + M * m)) ~ O (5N + 2(N * n)) (since M will be always <= N and m <= n)
# ~ O(N + N * n) ~ O(N*n) ~ O(N * n) time
#where
